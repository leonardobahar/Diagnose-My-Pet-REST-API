...
export class Dao{
				...
				this.mysqlConn.query(query, [patient.patient_name, patient.breed, patient.animal_type, patient.pet_owner], (err, res)=>{
					if	(err){
						...
						if (res.length>0){
							...
							query="INSERT INTO `patients`(`patient_name`,`animal_type_id`,`breed`,`weight`,`is_sterilized`,`description`,`patient_gender`,`birthdate`,`pet_owner_id`,`patient_picture`) VALUES(?,?,?,?,?,?,?,?,?,?)"
							this.mysqlConn.query(query,[patient.patient_name,patient.animal_type,patient.breed, patient.weight, patient.is_sterilized, patient.description,patient.gender,patient.birthdate,patient.pet_owner,patient.picture],(err,res)=>{
								if(err){
									reject(err)
									return
								}
		...
		return new Promise((resolve, reject)=>{
			if(!patient instanceof Patient){
				...
				if(patient.picture==='No Attachment'){
					const query="UPDATE patients SET patient_name=?,animal_type_id=?,breed=?,weight=?,is_sterilized=?,description=?,patient_gender=?,birthdate=?,pet_owner_id=? WHERE id=?"
					this.mysqlConn.query(query, [patient.patient_name,patient.animal_type,patient.breed,patient.weight,patient.is_sterilized,patient.description,patient.gender,patient.birthdate,patient.pet_owner,patient.id], (err, res)=>{
						if(err){
							reject(err)
							return
						}
				...
				const query="UPDATE patients SET patient_name=?,animal_type_id=?,breed=?,weight=?,is_sterilized=?,description=?,patient_gender=?,birthdate=?,pet_owner_id=?,patient_picture=? WHERE id=?"
				this.mysqlConn.query(query, [patient.patient_name,patient.animal_type,patient.breed,patient.weight,patient.is_sterilized,patient.description,patient.gender,patient.birthdate,patient.pet_owner,patient.picture,patient.id], (err, res)=>{
					if(err){
						reject(err)
						return
					}
	...
	addMedicalRecord(record){
		return new Promise((resolve,reject)=>{
			if(record instanceof MedicalRecords){
				const query="INSERT INTO `medical_records` (`description`, `medication`, `recorded_temperature`, `date_created`, `appointment_id`, `file`) VALUES(?, ?, ?, NOW(), ?, ?)"
				this.mysqlConn.query(query, [record.description, record.medication, record.recorded_temperature, record.appointment_id, record.file],(error, result)=>{
					if(error){
						reject(error)
						return
					}
		...
		return new Promise((resolve,reject)=>{
			if(record instanceof MedicalRecords){
				if(record.file==='No Attachment'){
					const query="UPDATE medical_records SET description=?, medication=?, recorded_temperature=?, date_created=NOW(), appointment_id=? WHERE id=?"
					this.mysqlConn.query(query, [record.description,record.medication, record.recorded_temperature, record.appointment_id, record.id], (error,result)=>{
						if(error){
							reject(error)
							return
						}
					...
					const query="UPDATE medical_records SET description=?, medication=?, recorded_temperature=?, date_created=NOW(), appointment_id=?, file=? WHERE id=?"
					this.mysqlConn.query(query, [record.description,record.medication, record.recorded_temperature, record.appointment_id, record.file, record.id], (error,result)=>{
						if(error){
							reject(error)
							return
						}
	...
	retrieveBookedAppointmentSchedule(){
		return new Promise((resolve,reject)=>{
			const query="SELECT a.id, a.start_time, a.end_time, a.proof_of_payment, a.description, a.additional_storage, a.status, a.doctor_id, a.co_doctor_id, cd.doctor_name AS co_doctor_name, d.doctor_name, a.patient_id, p.patient_name, p.birthdate, p.breed, p.pet_owner_id, u.user_name, u.mobile, a.booking_type_name, b.duration " +
				"FROM v2_appointment_schedule a INNER JOIN v2_booking_type b ON a.booking_type_name = b.booking_type_name INNER JOIN doctor d ON a.doctor_id=d.id LEFT OUTER JOIN doctor cd ON a.co_doctor_id=cd.id LEFT OUTER JOIN patients p ON a.patient_id=p.id LEFT OUTER JOIN users u ON u.id=p.pet_owner_id WHERE a.patient_id IS NOT NULL "+
			...
			this.mysqlConn.query(query, (error,result)=>{
				...
				const schedule=result.map(rowDataPacket=>{
					...
					return{
						...
						doctor_id:rowDataPacket.doctor_id,
						doctor_name:rowDataPacket.doctor_name,
						co_doctor_id: rowDataPacket.co_doctor_id,
						co_doctor_name: rowDataPacket.co_doctor_name,
						patient_id:rowDataPacket.patient_id,
						patient_name:rowDataPacket.patient_name,
						breed:rowDataPacket.breed,
						birthdate:d1,
	...
	retrieveAvailableAppointmentScheduleForDoctorDay(start_time, end_time, doctor_id){
		return new Promise((resolve,reject)=>{
			const query="SELECT a.id, a.start_time, a.end_time, a.proof_of_payment, a.description, a.additional_storage, a.status, a.doctor_id, d.doctor_name, a.co_doctor_id, cd.doctor_name AS co_doctor_name, a.patient_id, p.patient_name, p.pet_owner_id, u.user_name, a.booking_type_name, b.duration " +
				"FROM v2_appointment_schedule a LEFT OUTER JOIN v2_booking_type b ON a.booking_type_name = b.booking_type_name INNER JOIN doctor d ON a.doctor_id=d.id LEFT OUTER JOIN doctor cd ON a.co_doctor_id = cd.id LEFT OUTER JOIN patients p ON a.patient_id=p.id LEFT OUTER JOIN users u ON u.id=p.pet_owner_id " +
				"WHERE a.start_time >= ? AND a.end_time <= ? AND (d.id = ? OR a.co_doctor_id = ?) AND a.patient_id IS NULL OR a.booking_type_name IS NULL "+
				"ORDER BY a.start_time ASC "
			this.mysqlConn.query(query, [start_time, end_time, doctor_id, doctor_id],(error,result)=>{
				if(error){
					reject(error)
					return
				}
	...
	retrieveOneAppointmentSchedule(id){
		return new Promise((resolve,reject)=>{
			const query="SELECT a.id, a.start_time, a.end_time, a.proof_of_payment, a.description, a.additional_storage, a.status, a.doctor_id, d.doctor_name, a.co_doctor_id, cd.doctor_name as co_doctor_name, a.patient_id, p.patient_name, p.birthdate, p.breed, p.pet_owner_id, u.user_name, u.mobile, a.booking_type_name, bt.duration, " +
				...
				"FROM v2_appointment_schedule a LEFT OUTER JOIN doctor d ON a.doctor_id=d.id LEFT OUTER JOIN doctor cd ON a.co_doctor_id = cd.id LEFT OUTER JOIN patients p ON a.patient_id=p.id LEFT OUTER JOIN v2_booking_type bt ON bt.booking_type_name=a.booking_type_name LEFT OUTER JOIN users u ON u.id=p.pet_owner_id " +
	...
	retrieveAppointmentScheduleByStartTimeEndTimeDoctorId(start_time,end_time,doctor_id){
		return new Promise((resolve,reject)=>{
			...
			const query="SELECT a.id, a.start_time, a.end_time, a.proof_of_payment, a.description, a.additional_storage, a.status, a.doctor_id, d.doctor_name, a.co_doctor_id, cd.doctor_name AS co_doctor_name, a.patient_id, p.patient_name, p.birthdate, p.breed, p.pet_owner_id, u.user_name, u.mobile, a.booking_type_name, bt.duration " +
				...
				"WHERE a.patient_id IS NOT NULL AND a.start_time >= ? AND a.end_time <= ? AND (a.doctor_id=? OR a.co_doctor_id = ?)"+
				"ORDER BY a.start_time ASC "
			this.mysqlConn.query(query,[start_time,end_time,doctor_id, doctor_id],(error,result)=>{
				if(error){
					reject(error)
	...
	addAppointmentSlot(start_time, end_time, description, additional_storage, status, doctor_id, co_doctor_id, booking_type_name){
		return new Promise((resolve, reject)=>{
			...
			let query = "SELECT * FROM `v2_appointment_schedule` WHERE start_time >= ? AND (doctor_id = ? OR co_doctor_id = ? OR doctor_id = ? OR co_doctor_id = ?)"
			this.mysqlConn.query(query, [start_time, doctor_id, co_doctor_id, co_doctor_id, doctor_id], (err, res)=>{
				if (res.length > 0){
